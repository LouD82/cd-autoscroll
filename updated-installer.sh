...
# Claude Desktop Auto-Scroll Fix Installer (Improved Version)
# This script automatically patches the Claude Desktop app to add auto-scrolling functionality

# Exit on any error
set -e

echo "Claude Desktop Auto-Scroll Fix Installer (Improved Version)"
echo "--------------------------------------------------------"
...
mkdir -p "$BASE_DIR/extracted-fixed"
...
npx asar extract "/Applications/Claude.app/Contents/Resources/app.asar" "$BASE_DIR/extracted-fixed"

# Inject auto-scroll script
echo "Injecting auto-scroll script..."
MAIN_WINDOW_INDEX="$BASE_DIR/extracted-fixed/.vite/renderer/main_window/index.html"
...
AUTO_SCROLL_SCRIPT='  <body class="light">
    <script>
      // Safely load our auto-scroll script after everything else is loaded
...
(function() {
  ...
  window.addEventListener("load", () => {
    setTimeout(initAutoScroll, 2000);
            ...
            console.log("Auto-scroll fix script loaded successfully");
          } catch(e) {
            console.error("Error loading auto-scroll fix:", e);
          }
...
# Replace the end of the file with our script
sed -i '' 's#  <body class="light"></body>\n</html>#'"$AUTO_SCROLL_SCRIPT"'#' "$MAIN_WINDOW_INDEX"

# Pack modified files back into app.asar
echo "Packing modified files back into app.asar..."
npx asar pack "$BASE_DIR/extracted-fixed" "$BASE_DIR/app-fixed.asar"

# Replace original app.asar with the modified one
echo "Installing modified app.asar..."
cp "$BASE_DIR/app-fixed.asar" "/Applications/Claude.app/Contents/Resources/app.asar"
